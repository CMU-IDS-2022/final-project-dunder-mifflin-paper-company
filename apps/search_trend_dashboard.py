...
SEARCH_STRINGS = \
    ["fever", "chills", "cough", \
     "shallow_breathing", "shortness_of_breath", \
     "fatigue", "headache", \
     "cluster_headache", "sore_throat", \
     "nasal_congestion", "nausea", \
     "vomiting", "diarrhea", \
     "chest_pain", "burning_chest_pain", "back_pain"] 
...
def compute_correlation(search_trend_df, cases_df, shift, location_key, search_strings):
    ...
    location_search_trends = search_trend_df[search_trend_df['location_key'] == location_key]
...
def plot_dashboard(search_trend_df, cases_df, polygons):
    ...
    with col1:
        search_strings = st.multiselect('Search terms: (default: all)', SEARCH_STRINGS, default = ["fever"])
    with col2:
        st.markdown("<br /><br /><br />", unsafe_allow_html=True)
        shift = st.slider('Shift', 0, 30)

    search_strings = list(map(lambda search_string : "search_trends_" + search_string, search_strings))
    ...
    for ind, ele in enumerate(polygons["features"]):
        ...
        correlation = compute_correlation(search_trend_df, cases_df, shift, location_key, search_strings)
