...
LOCATION_ABBREVIATION_MAP = {
    ...
    "DISTRICT OF COLUMBIA": "US_DC", "FLORIDA": "US_FL", "GEORGIA": "US_GA", "GUAM": "US_GU", \
    "HAWAII": "US_HI", "IDAHO": "US_ID", "ILLINOIS": "US_IL", "INDIANA": "US_IN", "IOWA": "US_IA",\
    "KANSAS": "US_KS", "KENTUCKY": "US_KY", "LOUISIANA": "US_LA", "MAINE": "US_ME", "MARYLAND": "US_MD", \
    "MASSACHUSETTS": "US_MA", "MICHIGAN": "US_MI", "MINNESOTA": "US_MN", "MISSISSIPPI": "US_MS", \
    "MISSOURI": "US_MO", "MONTANA": "US_MT", "NEBRASKA": "US_NE", "NEVADA": "US_NV", "NEW HAMPSHIRE": "US_NH", \
    "NEW JERSEY": "US_NJ", "NEW MEXICO": "US_NM", "NEW YORK": "US_NY", "NORTH CAROLINA": "US_NC", \
    "NORTH DAKOTA": "US_ND", "NORTHERN MARIANA IS": "US_MP", "OHIO": "US_OH", "OKLAHOMA": "US_OK", \
    "OREGON": "US_OR", "PENNSYLVANIA": "US_PA", "PUERTO RICO": "US_PR", "RHODE ISLAND": "US_RI", \
    "SOUTH CAROLINA": "US_SC", "SOUTH DAKOTA": "US_SD", "TENNESSEE": "US_TN", "TEXAS": "US_TX", \
    "UTAH": "US_UT", "VERMONT": "US_VT", "VIRGINIA": "US_VA", "VIRGIN ISLANDS": "US_VI", "WASHINGTON": "US_WA", \
...
@st.cache
def read_files():
    government_response_df = pd.read_csv("../data/oxford-government-response_filtered.csv")    
...
def compute_correlation(government_response_df, cases_df, shift, location_key, factors):
    ...
    location_cases = cases_df[cases_df['location_key'] == location_key][["date", "new_confirmed"]]
    ...
    location_factor_values = government_response_df[government_response_df['location_key'] == location_key]

    if len(factors):
        factor_values = location_factor_values[factors][ROLLING_WINDOW - 1:].sum(axis = 1)
    else:
...
def plot_dashboard(government_response_df, cases_df, polygons):
    ...
    with col1:
        factor_strings = st.multiselect('Government Response Factors:', FACTORS_MAP.keys(), default = ["Stay At Home Requirements"])
    ...
    for ind, ele in enumerate(polygons["features"]):
        ...
        correlation = compute_correlation(government_response_df, cases_df, shift, location_key, factors)
    ...
    location_factor_values = government_response_df[government_response_df['location_key'] == location]
    ...
    with col1:
        ...
        usa_map_folium = usa_map.to_streamlit(height = 500, width = 900, add_layer_control=True, bidirectional = True)
    ...
    cases_chart = alt.Chart(data).mark_line(opacity=1, color='#57A44C').encode(
        alt.X(
            'x',
            title = "Days",
        ),
    ...
    trends_chart = alt.Chart(data).mark_line(opacity=1, color = '#5276A7').encode(
        alt.X(
            'x',
            title = "Days",
    ...
    if not len(st.session_state["history"]["region"]) or \
       st.session_state["history"]["region"][-1] != st.session_state["selected_region"] or \
       st.session_state["history"]["factors"][-1] != set(factor_strings) or \
       st.session_state["history"]["shift"][-1] != shift or \
       st.session_state["history"]["correlation"][-1] != corref:

        st.session_state["history"]["region"].append(st.session_state["selected_region"])
    ...
    historical_chart = alt.Chart(
        historical_data.reset_index(), title="Search history").mark_line(point=True, color="#FFFFFF").encode(
        alt.Y(
            'correlation',
            title = "Correlation",
        ...
        alt.X(
            'index',
            title = "History",
            scale = alt.Scale(domain=[-1, len(st.session_state["history"]["correlation"])]),
            axis = None
        ),
        tooltip = [
